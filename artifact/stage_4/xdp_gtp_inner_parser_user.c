...
/* Benchmarking toggle: when set (DISABLE_OFFLOAD=1 in the environment),
 * every learning/populate function below still runs -- F1AP/NGAP
 * watchers still learn, parsing/logging is byte-for-byte identical --
 * but none of the three functions that actually WRITE a real (non-
 * placeholder) entry into session_map/ul_session_map do so. Since the
 * kernel's offload logic strictly requires a map hit (and, for uplink,
 * ready == 1) before it will rewrite/redirect anything, a program that
 * never populates real entries naturally never offloads a single
 * packet -- every packet takes the same XDP_PASS path it always would
 * without this program attached at all, i.e. the real "normal CU-UP
 * processing path" for a fair A/B benchmark comparison. */
static int g_disable_offload;
...
static void maybe_populate_ul_session_map(__u32 ul_teid)
{
    ...
    if (g_disable_offload)
        return; /* benchmarking: see g_disable_offload's comment. */
...
static void maybe_learn_and_populate(const struct gtp_event *event)
{
    ...
    if (g_disable_offload)
        return; /* benchmarking: never populate session_map -- see
                 * g_disable_offload's comment. */
...
int main(int argc, char **argv)
{
    ...
    g_disable_offload = getenv("DISABLE_OFFLOAD") != NULL;
    if (g_disable_offload)
        printf("DISABLE_OFFLOAD set: every packet will take the normal "
               "(non-fast-path) route this run -- benchmarking baseline mode.\n");
     ...
     * UEContextSetupResponse to observe). Remove once no longer needed. */
    {
        ...
        if (seed && g_disable_offload) {
            printf("SEED_SESSION set but DISABLE_OFFLOAD is also set -- "
                   "ignoring SEED_SESSION for this benchmarking run.\n");
        } else if (seed) {
