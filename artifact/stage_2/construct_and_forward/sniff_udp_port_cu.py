...
def main():
    if len(sys.argv) not in (3, 4, 5):
        print(f"Usage: {sys.argv[0]} <iface> <udp_port> [expected_src_ip] [expected_teid_hex]")
        print(f"  e.g.: {sys.argv[0]} eth0 2153 192.168.71.150 0x57d1474a")
        sys.exit(1)

    iface = sys.argv[1]
    target_port = int(sys.argv[2])
    expected_src_ip = sys.argv[3] if len(sys.argv) >= 4 else None
    expected_teid = int(sys.argv[4], 16) if len(sys.argv) == 5 else None
    ...
    while True:
        ...
        try:
            ...
            teid_str = "?"
            pdcp_region = b""
            gtp_optional_present = False
            if len(payload) >= 8:
                gtp_flags = payload[0]
                gtp_msg_type = payload[1]
                gtp_teid = struct.unpack("!I", payload[4:8])[0]

                if expected_teid is not None and gtp_teid != expected_teid:
                    count -= 1  # don't count/print packets we're filtering out
                    continue
            ...
            if len(payload) >= 8:
                if gtp_optional_present:
                    print("  Framing check: E/S/PN set -- this still looks like the "
                          "OLD/broken N3-style framing (expects a GTP-U extension "
                          "header). If this is a CU->DU packet, the fix did not "
                          "take effect for it.")
                else:
                    print("  Framing check: E=0, S=0, PN=0 -- matches the FIXED F1-U "
                          "framing (no GTP-U extension header expected). The next "
                          "4 bytes below should be [PDCP: 3 bytes][SDAP: 1 byte], "
                          "then the inner IP packet should start immediately after.")
            ...
            print("!!! Exception while processing a packet -- capture continues, "
                  "but see traceback below (this would previously have silently "
