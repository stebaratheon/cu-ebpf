...
submit:
    /*
     * ISOLATION TEST VERSION: this file deliberately reverts JUST the
     * packet-body rewrite back to the old, already-proven-reliable Step-1
     * logic (simple in-place field swap, no bpf_xdp_adjust_tail, old
     * N3-style extension header left intact) -- while keeping everything
     * else (F1AP learning, session_map, MAC resolution) identical to the
     * real fixed version. Purpose: determine whether bpf_xdp_adjust_tail
     * + payload compaction is what's causing every packet after the
     * first (always-a-miss) one to go missing. If THIS version reliably
     * delivers every packet continuously, that confirms the resize logic
     * specifically is the problem, not redirect/session_map in general.
     */
    if (event->is_gtpu) {
