...
submit:
    /*
     * Step-1 fast-path offload: for GTP-U packets whose TEID has a
     * session_map entry, rewrite the outer L2/IP/UDP/GTP headers in place
     * and redirect out the configured egress interface instead of handing
     * the packet to the normal slow path. A map miss (the common case
     * today, since the map starts empty and is populated by hand) falls
     * straight through to the original XDP_PASS behavior below.
     *
     * This only rewrites fixed-size fields (same byte length in and out),
     * so no bpf_xdp_adjust_head/tail is needed and data/data_end stay valid.
     */
    if (event->is_gtpu) {
...
#pragma unroll
            ...
            /*
             * UDP checksum: dst IP (pseudo header), dst port, and TEID
             * (payload) all changed. Rather than patching each with an
             * incremental bpf_csum_diff() delta, set the checksum to 0 --
             * explicitly legal for UDP-over-IPv4 per RFC 768 ("0 means not
             * computed"), and a common choice in real GTP-U deployments
             * for exactly this performance reason. A version needing
             * checksummed UDP would apply bpf_csum_diff() deltas here
             * instead.
             */
            udp->check = 0;
